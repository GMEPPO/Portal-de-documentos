...
Se abrires o `index.html` diretamente (duplo clique), o navegador abre com o protocolo `file://`. Por segurança, **não permite** que a página carregue outros ficheiros locais (como o PDF na pasta `docs/`). Por isso a função "Buscar neste documento" pode falhar.
...
Para documentos que têm ficheiro PDF (ex.: Manual Web EPPO), coloque o PDF na pasta **`docs/`** com o nome esperado:

- Manual Web EPPO: `docs/MA-EPPO-WEB EPPO-V000.pdf`
...
Para os documentos Word, copie os ficheiros para a pasta **`docs/`**:

- **Guias de Preparação:** `docs/GUIAS DE PREPARAÇÃO - INFORMAÇÕES PARA DOCUMENTOS.docx`
- **Pagamento de Encomendas MB:** `docs/PAGAMENTO DE ENCOMENDAS COM REFERÊNCIAS MB (Fev.24).docx`

Na página de cada documento, além de "Abrir" e "Descarregar", aparece o botão **"Mostrar previsualização"**, que carrega o .docx e mostra o conteúdo em HTML na própria página. **Requer** que a aplicação seja aberta por um servidor local (ex.: `npx serve .`); com `file://` o browser não consegue carregar o ficheiro.
