...
export function extractPdfSearchTextFromBuffer(buffer: Buffer) {
...
export async function extractPdfSearchTextFromFile(file: File) {
  const arrayBuffer = await file.arrayBuffer();
  return extractPdfSearchTextFromBuffer(Buffer.from(arrayBuffer));
}
